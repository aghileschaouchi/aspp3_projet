...
%type   <ast_t> Foret Arbre A_contenu Quoted_text
...
%type   <dir_t> Import_path
%type   <int_t> NUM Points
...
Application:            Id_var Func_args                            {   struct ast * a = $2;
                                                                        while(a->node->app->fun != NULL)
                                                                            a = a->node->app->fun;
                                                                        a->node->app->fun = strcmp($1, "emit") == 0 ? mk_binop(EMIT) : mk_var($1);
                                                                        $$ = $2;
                                                                    }
...
Import:                 '$' Import_path FLECHE Id_var                   {   struct dir * d = $2;
                                                                            while (d->dir != NULL)
                                                                                d = d->dir;
                                                                            d->dir = mk_dir($4, DECLNAME, NULL);
                                                                            $$ = mk_import(mk_path(0, $2));
                                                                        }
                |       '$' Points Import_path FLECHE Id_var            {   struct dir * d = $3;
                                                                            while (d->dir != NULL)
                                                                                d = d->dir;
                                                                            d->dir = mk_dir($5, DECLNAME, NULL);
                                                                            $$ = mk_import(mk_path($2, $3));
                                                                        }
                ;

Import_path:            Id_var '/' Import_path                          { $$ = mk_dir($1, DIR, $3);}
                |       Id_var                                          { $$ = mk_dir($1, FILENAME, NULL);}
                ;
